...
def find_epoxy(atom0,atoms,bonds):
    ...
    neighbor_atoms = find_neighbors(atom0,bonds)
    # don't count H atoms bonded to N atoms
    # but DO count xlinker atoms
    active_atom = atom0
    chain = []
    chain.append(active_atom)
    for inl in neighbor_atoms:
        if atoms[inl]['type'] == 9:
            num_links = num_links + 1
            chain.append(inl)
        elif atoms[inl]['type'] == 12:
            num_links = num_links + 1  
            chain.append(inl)
    # now count all non H-bonds- including the with xlink atoms
    legit_atoms = []
    for ila in xrange(len(neighbor_atoms)):
        if atoms[neighbor_atoms[ila]]['type'] != 7:
            legit_atoms.append(neighbor_atoms[ila])
    ...
    #chain = []
    #chain.append(active_atom)
    while num_links < len(legit_atoms):  # iterate through neighbor bonds until all the xlinks chain ends are accounted for
        current_neighbors = find_neighbors(active_atom,bonds)
        # add a statement that checks to see if only current neighbors are already in the chain if so then break
        unique_atoms = set(current_neighbors).difference(chain)
        new_atoms = []
...
def main(argv=None):
    ...
    coords_file = "set6_x6_eq_coords.txt"
    ...
    bonds_file = "set6_x6_eq_bonds.txt"
        ...
        if int(bond['type']) == 13:
            ...
            try:
                ...
                if atoms[bond['atom1']]['type'] == 11:  # this is a N atom in an xlink molecule
                    ...
                    xlinks[xcount]['id'] = xcount
                    xlinks[xcount]['type'] = int(bond['type'])
                    xlinks[xcount]['atom1'] = int(bond['atom2'])
                    #xlinks[xcount]['atom2'] = int(bond['atom2'])                    
                    for ix in xrange(len(neighbor_atoms)):
                        new_int = ix + 2
                        tmp_str = "atom" + str(new_int)
...
# TO DO
# make the xlinks output make sense
# get rid of redundant entries and PBC crossings
    
    stoo = np.where(atoms['x']!=0)
    xmin = 1000
    xmax = -1000
    for xd in stoo[0]:
        if atoms[xd]['x'] < xmin:
    ...
    diff_lim = box_height/4
    count = 1
    for xl in xrange(len(xlinks)):
        for ixl in ['atom2','atom3','atom4']:
            if xlinks[xl][ixl] != 0:
                if ((xlinks[xl]['atom1'] == 21950) or (xlinks[xl]['atom1'] == 21960)):
                    print "WTF"
                xdiff = np.abs(atoms[xlinks[xl]['atom1']]['x'] - atoms[xlinks[xl][ixl]]['x'])
                ydiff = np.abs(atoms[xlinks[xl]['atom1']]['y'] - atoms[xlinks[xl][ixl]]['y'])
                zdiff = np.abs(atoms[xlinks[xl]['atom1']]['z'] - atoms[xlinks[xl][ixl]]['z'])
                if ((xdiff > diff_lim) or (ydiff > diff_lim) or (zdiff > diff_lim)):
                    #print "PBC?"
                    next
                if ((xdiff < diff_lim) and (ydiff < diff_lim) and (zdiff < diff_lim)):
                    #print xlinks[xl]['atom1']-1,xlinks[xl][ixl]-1
                    print count,"13",xlinks[xl]['atom1'],xlinks[xl][ixl]
                    count = count + 1
